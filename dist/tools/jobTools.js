...
exports.registerJobTools = registerJobTools;
...
function formatReward(amountWei, token) {
    if (!amountWei)
        return 'N/A';
    try {
...
function registerJobTools(server) {
    server.tool('search_jobs', 'Search for available jobs — tweet engagement campaigns with crypto rewards. Returns a list of active jobs you can complete to earn rewards.', {
        type: zod_1.z.enum(['boost', 'hire', 'all']).optional().describe('Job type: boost (engage with tweets), hire (content creation), or all (default: all)'),
        status: zod_1.z.enum(['active', 'completed', 'expired']).optional().describe('Filter by status (default: active)'),
        sort: zod_1.z.enum(['created_at', 'reward', 'deadline']).optional().describe('Sort field (default: created_at)'),
        limit: zod_1.z.number().optional().describe('Max results (default: 10)'),
        keyword: zod_1.z.string().optional().describe('Search keyword'),
        endingSoon: zod_1.z.boolean().optional().describe('Only show jobs ending within 24h'),
        token: zod_1.z.string().optional().describe('Filter by reward token (ETH, USDT, etc.)'),
    }, async (params) => {
        ...
        if (params.endingSoon)
            searchParams.set('ending_soon', 'true');
        if (params.token)
        ...
        try {
            ...
            if (allJobs.length === 0) {
                return {
                    content: [{ type: 'text', text: JSON.stringify({ success: true, data: { jobs: [], total: 0, message: 'No jobs found' } }, null, 2) }],
                };
            }
            const formatted = allJobs.map((job) => ({
                id: job.id,
            ...
            return {
                content: [{ type: 'text', text: JSON.stringify({ success: true, data: { jobs: formatted, total: formatted.length } }, null, 2) }],
            };
        }
        catch (e) {
            return {
                content: [{ type: 'text', text: JSON.stringify({ success: false, error: `Failed to fetch jobs: ${e.message}` }, null, 2) }],
                isError: true,
            };
        }
    });
