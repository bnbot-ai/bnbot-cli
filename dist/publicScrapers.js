...
exports.PUBLIC_SCRAPERS = {
    ...
    'fetch-hackernews-top': async (p) => {
        ...
        return stories.map((s, i) => ({ rank: i + 1, title: s.title, score: s.score, author: s.by, url: s.url || `https://news.ycombinator.com/item?id=${s.id}` }));
    },
    ...
    'fetch-v2ex-latest': async () => {
        ...
        return (data || []).map((t, i) => ({ rank: i + 1, title: t.title, replies: t.replies, node: t.node?.title, url: t.url }));
    },
    // ── StackOverflow — hot ──────────────────────────────────
    'fetch-stackoverflow-hot': async (p) => {
        ...
        return (data.items || []).map((i, idx) => ({ rank: idx + 1, title: i.title, score: i.score, answers: i.answer_count, url: i.link }));
    },
    // ── Wikipedia — summary ──────────────────────────────────
    ...
    'fetch-xiaoyuzhou-episodes': async (p) => {
        ...
        return episodes.slice(0, Number(p.limit) || 20).map((e, i) => ({
